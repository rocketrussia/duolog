    ...
    <>
      {!testOn && (
        <div>
          ...
          <p className="about">Приложение помогает подготовиться к интервью или экзамену</p>
          <form>
            <label>Выбирете готовый тест или создайте свой</label>
            <p></p>
            <select size="4">
              <option onClick={() => handleSelect('JavaScript')}>JavaScript</option>
              <option onClick={() => handleSelect('React')}>React</option>
              <option onClick={() => handleSelect('Algorithms')}>Algorithms</option>
              <option onClick={() => handleSelect('UserTest')}>Создать свой тест</option>
            </select>
            <p></p>
            {select === 'UserTest' ? (<textarea
              id="questions"
              name="questions"
              rows="20"
              wrap="off"
...
Каждый вопрос с новой строки"
            ...
            <p><button
              ...
              title={buttonTitle()}
              disabled={buttonOff}
              onClick={() => displayTest()}>Начать интервью</button></p>
          </form>
        </div>
      )}
